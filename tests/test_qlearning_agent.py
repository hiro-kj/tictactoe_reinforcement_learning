...
def test_qlearning_with_greedy(agent):
    # make it greedy
    agent.set_epsilon(0)

    # try 1-30
    agent.learn([1, 2], 1, 1, [10, 30])

    assert agent.select_action([1, 2]) == 1

    agent.learn([10, 30], 30, 30, [])

    assert agent.select_action([1, 2]) == 1
    assert agent.select_action([10, 30]) == 30

    # try 2-20
    agent.learn([1, 2], 2, 2, [20, 40])
    agent.learn([20, 40], 20, 20, [])
...
def test_qlearning_with_epsilon_greedy(agent):
